...
export type AuthContext = {
  userId: string;
  role: "TRAINEE" | "TRAINER" | "LOCAL_DIRECTOR" | "MAIN_DIRECTOR" | "SYSTEM_ADMIN";
...
const SCOPED_RELATIONS: Record<string, { relation: string; column: string }> = {
  applicationDocument: { relation: "application", column: "submittedFromMissionId" },
        ...
        async $allOperations({ model, operation, args, query }) {
          ...
          const missionFilter = directCol
            ? {
                OR: [
                  ...
                  ...(directCol === "scopedToMissionId" ? [{ [directCol]: null }] : []),
                ],
              }
            : {
                [relScope!.relation]: {
          ...
          switch (operation) {
            ...
            case "delete": {
              ...
              const target = await (prisma as never as Record<string, never>)[modelKey].findFirst({
