...
export function ApplicationExportButton({ applicationId }: { applicationId: string }) {
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 top-full z-20 mt-1 w-52 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg">
            <a
              href={`/api/export/trainer-application/pdf?id=${applicationId}`}
              target="_blank"
            ...
            <a
              href={`/api/export/trainer-application/xlsx?id=${applicationId}`}
              target="_blank"
              rel="noopener noreferrer"
              className="flex items-center gap-2 px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50"
              onClick={() => setOpen(false)}
