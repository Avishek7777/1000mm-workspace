...
function Block({ title, cols = 2, children }: { title: string; cols?: 1 | 2; children: React.ReactNode }) {
  return (
    <section className="mt-5 break-inside-avoid">
      <h2 className="mb-2 border-b-2 border-gray-800 pb-1 text-[13px] font-bold uppercase tracking-wide text-gray-800">{title}</h2>
      <div className={`grid gap-x-6 ${cols === 2 ? "grid-cols-2" : "grid-cols-1"}`}>{children}</div>
    </section>
  );
}

export default async function LmdApplicationPrintPage({
  params,
}: {
  ...
  const app = await prisma.application.findFirst({
    ...
    include: {
      ...
      recommendation: { include: { recommender: { select: { fullName: true } } } },
    },
  });
    ...
    <div className="mx-auto max-w-3xl">
      ...
      <div id="app-print" className="bg-white px-2 text-gray-900">
        ...
        <div className="flex items-start justify-between gap-4 border-b-4 border-gray-900 pb-3">
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-gray-500">1000 Missionary Movement — Bangladesh</p>
            <h1 className="mt-1 text-xl font-bold text-gray-900">{app.applicantFullName}</h1>
            <p className="mt-1 text-[11px] text-gray-600">
              {app.referenceNumber ? `Ref: ${app.referenceNumber} · ` : ""}
              {app.submittedFromMission.code} — {app.submittedFromMission.name} · {formatEnum(app.status)}
            </p>
            <p className="text-[11px] text-gray-600">Program: {app.window.program.title}</p>
          </div>
          {profilePhotoUrl && (
        ...
        <Block title="1. Personal Details">
          ...
          <Cell label="Full Name (Bangla)" value={app.applicantFullNameBangla} />
          <Cell label="Date of Birth" value={formatDate(app.applicantDateOfBirth)} />
          <Cell label="Gender" value={formatEnum(app.applicantGender)} />
          <Cell label="Blood Type" value={app.applicantBloodType?.replace("_", " ").replace("POS", "+").replace("NEG", "-")} />
          <Cell label="Marital Status" value={formatEnum(app.applicantMaritalStatus)} />
          <Cell label="Denomination" value={formatEnum(app.applicantDenomination)} />
          <Cell label="Place of Birth" value={app.applicantPlaceOfBirth} />
          <Cell label="Church Name" value={app.applicantChurchName} />
          <Cell label="Date of Baptism" value={formatDate(app.applicantDateOfBaptism)} />
          ...
          <Cell label="Permanent Address" value={permanentAddress || presentAddress || "—"} />
        </Block>
        ...
        <Block title="2. Family Details">
          ...
          <Cell label="Father — Religion" value={formatEnum(app.fatherReligion)} />
          ...
          <Cell label="Mother — Religion" value={formatEnum(app.motherReligion)} />
        ...
        <Block title="3. Educational Background" cols={1}>
          {educationEntries.length === 0 ? (
            <p className="py-1.5 text-[12px] text-gray-400">No education entries recorded.</p>
          ) : (
        ...
        {app.recommendation && (
          <Block title="LMD Recommendation" cols={1}>
            <div className="grid grid-cols-2 gap-x-6">
              <Cell label="Recommended By" value={app.recommendation.recommender.fullName} />
              <Cell label="Recommended On" value={formatDate(app.recommendation.recommendedAt)} />
            </div>
            {app.recommendation.writtenComment && (
              <div className="border-b border-gray-200 py-1.5">
                <p className="text-[9px] uppercase tracking-wider text-gray-500">Comment</p>
                <p className="whitespace-pre-wrap text-[12px] text-gray-900">{app.recommendation.writtenComment}</p>
              </div>
            )}
          </Block>
        )}
        ...
        <Block title="Documents on File" cols={1}>
          {app.documents.length === 0 ? (
            <p className="py-1.5 text-[12px] text-gray-400">No documents uploaded.</p>
          ) : (
            <ul className="text-[11px] text-gray-800">
              {app.documents.map((doc) => (
                <li key={doc.id} className="flex justify-between border-b border-gray-100 py-1">
        ...
        <div className="mt-10 grid grid-cols-2 gap-12 break-inside-avoid">
          <div className="border-t border-gray-400 pt-1 text-[10px] text-gray-600">LMD Signature</div>
          <div className="border-t border-gray-400 pt-1 text-[10px] text-gray-600">Date</div>
        </div>
        <p className="mt-6 text-center text-[9px] text-gray-400">Generated {generatedAt} · 1000MMBD Portal</p>
      </div>
    </div>
  );
}
