...
export default async function ComplaintDetailPage({
  ...
  // LMD can view non-anonymous complaints from their own mission
  let lmdMissionCode: string | null = null;
  if (isLmd) {
    const mission = await prisma.localMission.findFirst({
      where: { directorId: user.id },
      select: { code: true },
    });
    lmdMissionCode = mission?.code ?? null;
  }

  const isLmdMissionComplaint =
    isLmd && !complaint.isAnonymous && complaint.missionCode === lmdMissionCode;

  const canView = isStaff || isOwner || isLmdMissionComplaint;
  if (!canView) redirect("/dashboard/complaints");

  // Gate UD responding behind the setting
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      {/* Header card */}
      <div
        className={`rounded-xl border bg-white p-6 ${complaint.isResolved ? "border-teal-200" : "border-gray-200"}`}
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <div className="flex flex-wrap items-center gap-2">
            <span
              className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${CATEGORY_COLORS[complaint.category]}`}
            >
      ...
      {complaint.isResolved && complaint.response && (
        <div className="rounded-xl border border-teal-200 bg-teal-50 p-5">
          ...
          <p className="whitespace-pre-wrap text-sm text-teal-900 leading-relaxed">
            {complaint.response}
          </p>
      ...
      {!isStaff && !complaint.isResolved && (
        <div className="rounded-lg border border-gray-100 bg-gray-50 px-4 py-3 text-xs text-gray-500 text-center">
          Your complaint is being reviewed. You'll be notified when a response
          is available.
        </div>
      )}
    </div>
