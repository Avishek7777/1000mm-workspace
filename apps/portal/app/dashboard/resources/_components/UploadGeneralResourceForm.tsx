...
export function UploadGeneralResourceForm({ programs }: Props) {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-2 text-xs font-medium text-white hover:bg-teal-700 transition-colors">
  ...
  return (
    <div className="rounded-xl border border-teal-200 bg-teal-50 p-4">
      <div className="mb-3 flex items-center justify-between">
        <p className="text-sm font-semibold text-teal-800">Upload Resource</p>
      ...
      <form action={dispatch} className="space-y-3">
        ...
        <div>
          ...
          <input name="title" required className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-teal-400 focus:outline-none" placeholder="Resource title" />
        </div>

        <div>
          <label className="block text-xs font-medium text-gray-700 mb-1">Description</label>
          <input name="description" className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-teal-400 focus:outline-none" placeholder="Short description" />
        </div>
        ...
        <div>
          ...
          <select
            name="programId"
            className="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-teal-400 focus:outline-none"
          >
          ...
          <p className="mt-0.5 text-[10px] text-gray-400">Choose a specific program to restrict visibility to its enrolled trainees.</p>
        </div>
        ...
        <div className="flex justify-end gap-2 pt-1">
          ...
          <button type="submit" disabled={pending || !fileKey} className="rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-teal-700 disabled:opacity-60 transition-colors">
